...
def stream_video_pipeline(worker_id, video_path, ws_url, log_filename, size_min_kb, size_max_kb, max_frames=None):
    ...
    if not encoded_frames:
        print(f"[{worker_id:03d}] [NO_FRAMES] No frames could be encoded from: {video_path}")
        return

    print(f"[{worker_id:03d}] Pre-encoded {len(encoded_frames)} frames — connecting...")

    # ── CONNECT THEN PURE NETWORK TIMING LOOP ────────────────
    try:
        ws = websocket.create_connection(ws_url, timeout=10)
    except Exception as e:
        print(f"[{worker_id:03d}] [CONN_FAIL] Could not connect to stream socket: {e}")
        return
    ...
    try:
        for f in encoded_frames:
            ...
            strict_max_bytes = f["strict_max_bytes"]
            binary_bytes     = f["binary_bytes"]
            byte_size        = f["byte_size"]
            h_active, w_active = f["shape"][:2]

            # ==============================================================================
            # ISOLATED HIGH-PRECISION NETWORK TIMING WINDOW — zero CPU work inside
            # ==============================================================================
